...
class PlaysTV {
  ...
  searchVideos(searchParams, count=50, sort = "recent", sortdir = "desc") {
    ...
    if (searchParams.gameId) query.userId = searchParams.gameId
    if (searchParams.hashtags) query.userId = searchParams.hashtags.join(",")
    if (searchParams.metatags) query.userId = searchParams.metatags.join(",")
